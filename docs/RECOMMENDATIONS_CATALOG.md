...
- `CRR` flagging logic itself (but duration assumption keeps these LOW — see §7)
...
| **User-facing classification** | Bursty workloads (CV of hourly CPU ≥ 0.5 AND P95 ≥ 2×avg) use the tighter **P95 ≤ 40 %** target on the new SKU. Steady workloads use the relaxed **P95 ≤ 80 %**. |
...
| **Headroom** | Default `1.2×` multiplier on observed values when projecting onto the proposed SKU. |
...
| | |
|---|---|
| **Trigger** | Current SKU in D, E, or F family **and** avg CPU below the B-series baseline for the target vCPU count **and** P95 CPU below 2× baseline. |
...
| **Confidence** | MEDIUM → HIGH with enrichment |
| **CloudFit parity** | ✅ Logic 4 (inverse direction) |
...
|---|---|---|---|---|
| RSZ-DWN-001 | rightsize | Smaller SKU when CPU/mem low | MEDIUM | ✅ Logic 2 |
| RSZ-UPS-001 | rightsize | Larger SKU under sustained pressure | — | (reserved) |
| RSZ-BSF-001 | rightsize | D/E/F → B-series when bursty/low | MEDIUM | ✅ Logic 4 |
| RSZ-BSM-001 | rightsize | B-series → standard when over budget | MEDIUM | ✅ Logic 4 inv. |
...
| SWP-LFC-001 | swap | Retiring SKU → modern replacement | HIGH | (CloudOpt-only) |
| SWP-DST-001 | swap | Premium SSD → Standard SSD | — | (reserved) |
| SWP-DSK-001 | swap | Diskful → Diskless SKU | MEDIUM | ✅ Logic 3 |
| SWP-ARC-001 | swap | x64 → ARM64 candidate flag | DISCOVERY | (CloudOpt-only) |
| DCM-IDL-001 | decom | Idle running VM | MEDIUM | ✅ Logic 1 |
...
| CLN-DSK-001 | cleanup | Unattached disk ≥ 30 days | HIGH | ✅ Logic 5 |
...
| QTA-WRN-001 | quota | Quota warning (70–85 %) | HIGH | (CloudOpt-only) |
| QTA-CRI-001 | quota | Quota critical — individual | HIGH | (CloudOpt-only) |
| QTA-CRG-001 | quota | Quota critical — groupable | HIGH | (CloudOpt-only) |
...
|---|---|---|
| 30-minute (PT30M) metric granularity | Currently PT1H | PT30M would 2× data volume; PT1H is a deliberate trade-off |
